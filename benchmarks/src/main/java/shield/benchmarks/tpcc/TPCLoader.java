...
public class TPCLoader {
  ...
  public static void loadData(TPCCExperimentConfiguration tpccConfig, String expConfigFile)
    ...
    if (tpccConfig.LOAD_WAREHOUSES) {

    System.out.println("Loading [WAREHOUSES]");

    for (int i = tpccConfig.START_WAREHOUSE; i < tpccConfig.NB_WAREHOUSES; i += warehousesToLoad) {
    ...
    System.out.println("Finished Warehouses");
  ...
  private static void loadItemTable(int start, int end, int uid, String expConfigFile)
    ...
    ClientBase client = ClientUtils.createClient(config.CLIENT_TYPE, expConfigFile, uid, uid);

    client.registerClient();
    TPCGenerator generator = new TPCGenerator(client, config);
      ...
      while (!success) {
        ...
        try {
          ...
          for (int ij = 0; ij < 10; ij++) {
            ...
            client.write(TPCCConstants.kItemTable, iKey.str(), row);
          }
    ...
    //   System.out.println("[Warehouse]" + wid);
      ...
      while (!success) {
        ...
        //   System.out.println("[Wid]: " + wid);
          client.startTransaction();
          client.write(TPCCConstants.kWarehouseTable, wKey.str(), row);
      ...
      while (!success) {
        try {
          ...
          for (int did = 0; did < config.NB_DISTRICTS; did++) {
            ...
            client.write(TPCCConstants.kDistrictTable, dKey.str(), row);
          }
                ...
                while (!success) {
                  try {
                   ...
                   // String last = generator.RandString(9, 16, false);
                    ...
                    client.write(TPCCConstants.kCustomerTable, cKey.str(), row);

                    // Now need to update customer last name
                    CustomerByNameKey key = new CustomerByNameKey(wid,did,last);
                    List<byte[]> results =
                            client.readAndExecute(TPCCConstants.kCustomerByNameTable, key.str());
                    String update = null;
                    row = results.get(0);
                    try {
                    ...
                    client.write(TPCCConstants.kCustomerByNameTable, key.str(),row);
                    ...
                    client.write(TPCCConstants.kHistoryTable, hKey.str(), row);
                    client.commitTransaction();
                    success = true;
            ...
            while (!success) {
              try {
                ...
                //   System.out.println("[Stock] " + wid + " " + iid);
                  ...
                  client.write(TPCCConstants.kStockTable, sKey.str(), row);
                  ij++;
                }
                client.commitTransaction();
                success = true;
      ...
      while (!success) {
        try {
          ...
          for (int j = 0; j < config.NB_DISTRICTS; j++) {
            ...
            client.write(TPCCConstants.kEarliestNewOrderTable, eolKey.str(), row);
          }
          client.commitTransaction();
          success = true;
        } catch (DatabaseAbortException e) {
            ...
            //   System.out.println("[OrderLine/NewOrder] wid " + wid + " did " + d + " oid " + oid);
              while (!success) {
                try {
                  ...
                  client.write(TPCCConstants.kOrderByCustomerTable, obcKey.str(), row);
                  ...
                  client.write(TPCCConstants.kOrderTable, oKey.str(), row);
                  ...
                  for (int olid = 0; olid < olCnt; olid++) {
                    ...
                    client.write(TPCCConstants.kOrderLineTable, olKey.str(), row);
                  }
                  ...
                  if (oid >= config.INIT_NEW_ORDER_NB * 0.7) {
                    ...
                    client.write(TPCCConstants.kNewOrderTable, noKey.str(), row);
                  }
