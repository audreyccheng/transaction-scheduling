...
public class TPCGenerator {
  ...
  private final HashMap<String, Integer> customerStats;


  public TPCGenerator(ClientBase client, TPCCExperimentConfiguration config) {
    this.config = config;
    this.client = client;
    ...
    this.customerStats = new HashMap<>();
  }
  ...
  public TNewOrderTransaction GenerateNewOrderInput() {
    ...
    wid = Generator.generateInt(0, config.NB_WAREHOUSES);
   ...
   if (rollback) {
    ...
    for (int i = 0; i < olCnt; i++) {
      ...
      if (olSupplyWidRand!= 0) {
        ...
        // do {
          swid = Generator.generateInt(0,config.NB_WAREHOUSES);
        // } while (swid == wid && config.NB_WAREHOUSES > 1);
      }
      swids.add(swid);

    }
    //  System.out.println("GenerateNewOrderInput wid: " + wid + " did: " + did);
    return new TNewOrderTransaction(this, wid,did,cid,olCnt,iids,swids,olQuantities,txn_id++);

  }

  public TPaymentTransaction GeneratePaymentInput() {
    // System.out.println("GeneratePaymentInput S");
    int cWid;
    int cDid;
    int hAmount;
    ...
    int cid;
    String cName = null;
    int wid = Generator.generateInt(0, config.NB_WAREHOUSES);
    ...
    // System.out.println("GeneratePaymentInput 1");
  ...
  if (r < 85) {
    ...
    // use remote warehouse
    // do {
      cWid = Generator.generateInt(0, config.NB_WAREHOUSES);
    // } while (cWid == wid &&
   ...
   hAmount = Generator.generateInt(1, 500000);  // multiplied by 100
    // System.out.println("GeneratePaymentInput");
    return new TPaymentTransaction(this,cWid, cDid, cid, wid, did, hAmount,
        scanByLastName, cName, txn_id++);
  }

  public TOrderStatusTransaction GenerateOrderStatusInput() {
  ...
  boolean scanByLastName =
    ...
    // System.out.println("GenerateOrderStatus");
    return new TOrderStatusTransaction(this, wid,cid,did,scanByLastName, cName);
  }

  public TDeliveryTransaction GenerateDeliveryInput() {
  int wid = Generator.generateInt(0, config.NB_WAREHOUSES- 1);
  int carrierId = Generator.generateInt(1, 10);
  return new TDeliveryTransaction(this, wid,carrierId,0);
  }

  TStockLevelTransaction GenerateStockLevelInput() {

  int wid = Generator.generateInt(0, config.NB_WAREHOUSES- 1);
  int did = Generator.generateInt(0, config.NB_DISTRICTS- 1);

  return new TStockLevelTransaction(this, wid,did);
  }

  private void createTPCCTables(ClientBase client) {
 ...
 */
 ...
 public String getElementAtIndex(byte[] currentList, int i) throws IndexOutOfBoundsException {
  ...
  public void runNextTransaction() {
    ...
    //   System.out.println("Generator runNextTransaction x:" + x);
...
//      System.out.println("Executed in: " + (end-begin));
  ...
  public void updateCustomer(String cust) {
    if (!customerStats.containsKey(cust)) {
      customerStats.put(cust, 1);
    } else {
      Integer count = customerStats.get(cust);
      customerStats.put(cust, count+1);
    }
  }

  public HashMap<String, Integer> getCustStats() {
    return customerStats;
  }

  public void printCustomers() {
    System.out.println("Total customers: " + customerStats.size());
    List<Integer> vals = new ArrayList<>(customerStats.values());
    Collections.sort(vals, Collections.reverseOrder());
    System.out.println("Customers: " + vals);
  }
