...
public class NNewOrderTransaction extends BenchmarkTransaction{
  ...
  public NNewOrderTransaction(TPCGenerator generator, int wid, int did, int cid, int olCnt,
  ...
  public boolean tryRun() {
    try {
      ...
      int dNextOid=0;
      List<byte[]> results;
      byte[] wRow = new byte[0];
      byte[] dRow = new byte[0];
      byte[] cRow = new byte[0];
      byte[] oRow = new byte[0];
      int cDiscount=0;
      String cLastName= "";
      String cCredit= "";
      int itemId=0;
      ...
      client.update(TPCCConstants.kDistrictTable, dKey.str(), dRow);
    ...
    //   System.out.println("NewOrder " + orderTable.printColumn(oRow));
      ...
      for (Integer iid : iids) {
        ...
        client.update(TPCCConstants.kStockTable, stockKey.str(), oRow);
