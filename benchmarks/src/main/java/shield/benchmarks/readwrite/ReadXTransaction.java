 ...
 * customer
 */
public class ReadXTransaction extends BenchmarkTransaction{
  private final RWExperimentConfiguration config;
  private Integer X;
  private Integer Z;

  public ReadXTransaction(RWGenerator generator, int X, int Z, long txn_id) {
    this.X = X;
    this.Z = Z;
...
//      System.out.println("ReadX Transaction");
      ...
      // Get Account
    // results = client.readAndExecute("", this.X.toString());
    ...
    //     return true;
    //   }
      results = client.readForUpdateAndExecute("", this.Z.toString());
     ...
     results = client.writeAndExecute("", this.Z.toString(), val);
