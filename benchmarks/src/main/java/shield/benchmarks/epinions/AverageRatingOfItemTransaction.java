...
public class AverageRatingOfItemTransaction extends BenchmarkTransaction {
    private EpinionsExperimentConfiguration config;
    private int iid;
    private long txn_id;

    public AverageRatingOfItemTransaction(EpinionsGenerator generator, int iid, long txn_id) {
        this.iid = iid;
        this.txn_id = txn_id;
...
//            System.out.println("AverageRatingOfItemTransaction");
            List<byte[]> results;
            ...
            if (idList.size() != 0) {
                System.out.println("AvgRI size: " + idList.size());
            }
